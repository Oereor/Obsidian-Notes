...
利用 $A$ 的顺序主子式 ([[方阵的主子式]]), 我们还有如下正定性的判定定理. 
...
> **证明.** *必要性.* 设 $A$ 正定. 对于 $1\leq k\leq n$, 将 $A$ 分块为如下形式 $$A=\begin{bmatrix}
    A_{k} & A_{k\times(n-k)}\\
    A_{(n-k)\times k} & A_{n-k}
\end{bmatrix}$$ 其中 $A_k$ 是 $k$ 阶主子矩阵. 取任意的 $$X=\begin{bmatrix}
    X_k \\
    O_{n-k}
\end{bmatrix}\in\mathbb{R}^n\qquad X\neq 0$$ 则有 $$\begin{align*}
    f(X)&=X^TAX=X_k^TA_kX_k+X_k^TA_{k\times(n-k)}O_{n-k}+O_{(n-k)\times k}^TA_{(n-k)\times k}X_k+O_{(n-k)\times(n-k)}^TA_{n-k}O_{n-k}\\
    &=X_k^TA_kX_k>0.
\end{align*}$$ 则 $A_k$ 正定. 
>
> *充分性.* 用数学归纳法. 当 $n=1$ 时命题显然成立, 现假定 $A$ 为 $n-1$ 阶时命题成立, 当 $A$ 为 $n$ 阶时, 将其分块为 $$A=\begin{bmatrix}
    A_{n-1} & \alpha\\
    \alpha^T & a_{nn}
\end{bmatrix}$$ 其中 $A_{n-1}$ 是 $n-1$ 阶主子矩阵, $\alpha$ 是 $n-1$ 维行向量, $a_{nn}$ 是标量. 由归纳假设, $A_{n-1}$ 的所有顺序主子式均大于零. 对 $A$ 作初等变换可得 $$\begin{bmatrix}
    A_{n-1} & \alpha\\
    \alpha^T & a_{nn}
\end{bmatrix}\sim \begin{bmatrix}
    A_{n-1} & 0\\
    0 & a_{nn}-\alpha^TA_{n-1}^{-1}\alpha
\end{bmatrix}$$ 即存在可逆矩阵 $P$ 使得 $$P^TAP=\begin{bmatrix}
    A_{n-1} & 0\\
    0 & a_{nn}-\alpha^TA_{n-1}^{-1}\alpha
\end{bmatrix}.$$ 那么只需证明 $P^TAP$ 正定. 显然 $|P^TAP|=|P|^2|A|>0$, 那么 $a_{nn}-\alpha^T A_{n-1}^{-1}\alpha>0$. 取任意的 $$X=\begin{bmatrix}
    X_{n-1} \\
    x_n
\end{bmatrix}\in\mathbb{R}^n\qquad X\neq 0$$ 则有 $$\begin{align*}
    f(X)&=X^T(P^TAP)X=X_{n-1}^TA_{n-1}X_{n-1}+x_n^2(a_{nn}-\alpha^TA_{n-1}^{-1}\alpha)\\
    &\geq x_n^2(a_{nn}-\alpha^TA_{n-1}^{-1}\alpha)>0.
\end{align*}$$ 由此可见 $P^TAP$ 正定, 进而 $A$ 正定. 

********

**例.** 设 $A$ 是实对称矩阵, 且 $A$ 正定. 证明: 存在正定矩阵 $B$ 使得 $A=B^2$. 

> **解.** 由于 $A$ 正定, 故 $A$ 的特征值均为正. 设 $A$ 的特征值为 $\lambda_1,\lambda_2,\dots,\lambda_n>0$, 则存在正交矩阵 $P$ 使得 $$P^TAP=\begin{bmatrix}
    \lambda_1 & & \\
    & \ddots & \\
    & & \lambda_n
\end{bmatrix}.$$ 令 $$B=P\begin{bmatrix}
    \sqrt{\lambda_1} & & \\
    & \ddots & \\
    & & \sqrt{\lambda_n}
\end{bmatrix}P^T,$$ 则显然 $B$ 正定, 且 $B^2=A$. 证毕.